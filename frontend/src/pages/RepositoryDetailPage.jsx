...
function buildFailedCheckCounts(scanResult) {
	...
	for (const check of didacticChecks) {
		...
		const checkId = resolveRepositoryCheckId({
			key: check?.checkId,
			label: check?.label,
		});
...
function RepositoryCheckLearnMoreLink({
	repositoryId,
	checkId,
	state,
	onClick,
...
function RepositoryDetailPage() {
	...
	const runGeneralScan = useCallback(async () => {
		...
		try {
			const scanResult = await requestGeneralScan(
				repositoriesUrl,
				repositoryId,
			);
			...
			const existingSnapshots = normalizeScanSnapshots(
				readJsonStorage(SCAN_CACHE_KEY, {}),
			);
			const nextSnapshots = {
				...existingSnapshots,
				[String(repositoryId)]: {
	...
	const didacticChecks = Array.isArray(activeResult?.didacticChecks)
		...
		: [];
	const greenChecks = didacticChecks.filter(
		(check) => check.status === "green",
	);
	const attentionChecks = didacticChecks.filter(
		(check) => check.status !== "green",
	);
	...
	return (
		<div className="page repository-detail-page">
			...
			{isLoadingSession ? (
				<Card
					title="Checking GitHub session"
					subtitle="Contacting backend /auth/me"
				>
			...
			{isMissingConfig ? (
				<Card
					...
					subtitle="Set VITE_API_URL for this environment"
				>
			...
			{hasSessionError ? (
				<Card
					title="Could not validate session"
			...
			{isUnauthenticated ? (
				...
				>
					<p className="state-note">
						Your current session is not authenticated. Return to onboarding and
						try connecting with GitHub again.
					</p>
			...
			{isAuthenticated && repositoryState.status === "not_found" ? (
				<Card>
					<p className="state-note state-note-danger">
						The selected repository was not found in your current public
						repository list.
					</p>
				</Card>
			) : null}
				...
				<>
					<Card title="Scan summary">
						<p className="scan-check-message">
							This report shows one contextual scan with didactic checks and
							clear next actions.
						</p>
						...
						{didacticChecks.length ? (
							<div className="report-line-list">
								{didacticChecks.map((check) => {
									...
									return (
										<div
											key={`didactic-${check.checkId}`}
											className="report-line-row"
										>
							...
							<p className="scan-check-message">
								No didactic checks were returned by this scan.
							</p>
						)}
					</Card>
				</>
