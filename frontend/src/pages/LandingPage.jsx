...
const CHECKLIST_OPTIONS = [
	...
	},
	{
		...
		description:
			"Defensive checks for secret handling and risky code patterns.",
	},
];
...
function buildSafeEvidenceFromFinding(finding, checkId) {
	return {
		...
		flaggedLineExplanation: normalizeBoundedString(
			finding?.flaggedLineExplanation,
			220,
		),
...
function buildCodeSafetyEvidenceByCheckId(scanResult) {
	...
	const evidencePacketFindings = Array.isArray(
	...
	const resultGroups = Array.isArray(scanResult.results)
		? scanResult.results
		: [];
...
function buildDidacticCopy(checkId, item) {
	const guide = getRepositoryCheckGuideById(checkId);
	const action =
		Array.isArray(guide?.howToFix) && guide.howToFix.length
			? guide.howToFix[0]
			: "Review this check and apply the recommended fix.";
	...
	return {
		...
		whyItMatters:
			guide?.whyMatters ||
			guide?.whyChecked ||
			"This signal helps keep repository quality and security understandable.",
		whatFound:
...
function buildGuideRouteRepositoryId(repository) {
	const provider =
		typeof repository?.provider === "string" ? repository.provider.trim() : "";
	const owner =
...
function buildRepositoryAnalyticsParams(repository) {
	...
	return {
		...(repositoryIsPublic && repositoryOwner
			? { repository_owner: repositoryOwner }
			: {}),
		...(repositoryIsPublic && repositoryName
...
function LandingPage() {
	...
	const scansUrl = useMemo(
		() => buildBackendUrl(apiBaseUrl, "/scans"),
		[apiBaseUrl],
	);
	...
	const executeScan = async () => {
		...
		const startRepositoryParams =
			buildRepositoryAnalyticsParams(repositoryTarget);
		trackScanStarted({
		...
		<div className="page scan-page">
			<Card className="scan-intro-card">
				...
				<p className="page-description">
					Paste a repository URL, choose at least one checklist, and RepoGuard
					will run a focused report.
				</p>
			</Card>
			...
			<Card className="scan-form-card">
				<form className="scan-form" onSubmit={handleSubmit}>
					...
					<fieldset className="scan-checklist-fieldset">
						<legend className="scan-field-label">
							Choose at least 1 checklist to analyze
						</legend>
								...
								return (
									<label key={option.id} className="scan-checklist-option">
										...
										<span className="scan-checklist-copy">
											<span className="scan-checklist-title">
												{option.label}
											</span>
											<span className="scan-checklist-description">
												{option.description}
											</span>
										</span>
									</label>
					...
					<div className="scan-actions">
						<Button
							type="submit"
							disabled={!canStartScan || scanState.status === "loading"}
						>
							Start scan
						</Button>
					</div>
											...
											{visibleGroupItems.map((item) => {
												...
												const isPriorityCheck =
													...
													(PRIORITY_REPOSITORY_HEALTH_CHECK_IDS.has(checkId) ||
														/dependabot|dependency automation/i.test(
															String(item.label || ""),
														));
												const didacticChecks = Array.isArray(
													scanState.result?.didacticChecks,
												)
													? scanState.result.didacticChecks
													: [];
												const didacticCheck = didacticChecks.find(
													(candidate) => candidate?.checkId === checkId,
												);
												const didacticCopy = didacticCheck
													? {
														whatChecked: didacticCheck.whatChecked,
														whyItMatters: didacticCheck.whyItMatters,
														whatFound: didacticCheck.whatFound,
														suggestedAction: didacticCheck.suggestedAction,
													}
													: buildDidacticCopy(checkId, item);
												const didacticSources =
													Array.isArray(didacticCheck?.sources) &&
													didacticCheck.sources.length
														? didacticCheck.sources
														: buildDidacticSources(checkId);
													...
													>
														<div className="scan-result-copy">
															<p className="scan-result-title">
																...
																<span className="scan-result-priority">
																	{DIDACTIC_STATUS_LABEL[item.status] ||
																		item.status}
																</span>
																{isPriorityCheck ? (
																	<span className="scan-result-priority">
															...
															{displayFilePath ? (
																<p className="scan-result-file">
																	File:{" "}
																	<span className="scan-result-file-path">
																		{displayFilePath}
																	</span>
																</p>
															) : null}
															<p className="scan-result-details">
																<strong>What was checked:</strong>{" "}
																{didacticCopy.whatChecked}
															</p>
															<p className="scan-result-details">
																<strong>Why it matters:</strong>{" "}
																{didacticCopy.whyItMatters}
															</p>
															<p className="scan-result-details">
																<strong>What RepoGuard found:</strong>{" "}
																{didacticCopy.whatFound}
															</p>
															<p className="scan-result-details">
																<strong>Suggested action:</strong>{" "}
																{didacticCopy.suggestedAction}
															</p>
															{didacticCheck?.confidence ? (
																<p className="scan-result-details">
																	<strong>Confidence:</strong> {didacticCheck.confidence}
																</p>
															) : null}
															{didacticCheck?.uncertaintyNote ? (
																<p className="scan-result-details">
																	<strong>Uncertainty:</strong> {didacticCheck.uncertaintyNote}
																</p>
															) : null}
															...
															<Link
																...
																state={{
																	...
																	checkStatus:
																		safeCodeEvidence?.status || item.status,
